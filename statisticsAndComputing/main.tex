...
In this case, the Type I error rate of asymptotic method may be far away from nominal level. 
...
For example, we consider the model $X_i=u_i\Bv$, $i=1,\ldots,n$, where
 $u_1,\ldots,u_n$ are iid random variables with $\myE u_1=0$, $\myE u_1^2=1$ and $\Bv\in\mathbb{R}^p$ is a vector.
In this case, 
\begin{equation*}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation*}
By law of large number, we have $\sum_{i=1}^n u_i^2/n\xrightarrow{P} 1$. By central limit theorem, we have $\sum_{i=1}^n u_i/\sqrt{n}\xrightarrow{\mathcal{L}}N(0,1)$. Then we have
$$
\frac{2T(X_1,\ldots,X_n)}{n\Bv^T \Bv }+1\xrightarrow{\mathcal{L}}\chi^2_1.
$$
Since  the asymptotic distribution of $T(X_1,\ldots, X_n)$ is not normal distribution, the asymptotic method does not have the correct level asymptotically.
For the randomization method, we have
\begin{equation*}
    \begin{aligned}
...
Next we show that $C_1$, $C_2$ and $C_3$ are negligible under the assumptions of the theorem.
    ...
    $$\myE(C_1^2)=\myE\myE(C_1^2|X_1,\ldots,X_n)=\sum_{j<i}\myE{(Z_i^T \Gamma^T \Gamma Z_j)}^2=\frac{n(n-1)}{2}\mytr \Sigma^2,$$
    and
