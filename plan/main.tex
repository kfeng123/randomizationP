...
\end{remark}
    ...
    On the other hand, if $r>0$, i.e. $\Sigma$ has a few spiked eigenvalues, $T(X_1,\ldots,X_n)$ is not asymptotically normal distributed.
    In this case, the asymptotic methods of~\cite{Bai1996Efiect} and~\cite{Chen2010A} are not directly applicable.
...
Let $z_{\alpha}$ be the $1-\alpha$ quantile of the randomization distribution.
...
\begin{proposition}\label{prop:spiked2}
    ...
    {\sqrt{\frac{n(n-1)}{2}\mytr(\Sigma^2)\big)}}
...
To prove~\eqref{lemma2R1}, we only need to show that the left hand side of~\eqref{lemma2R1} converges to $0$ in mean.
    ...
    \begin{equation*}
    \begin{aligned}
        &\myE\sum_{j<k}\big(\sum_{i:i>k} Y_{ij} Y_{ik}\big)^2
        =
        \myE\sum_{j<k}\Big(\sum_{i:i>k} Y_{ij}^2 Y_{ik}^2
        +2\sum_{i_1,i_2:i_1>i_2>k} Y_{i_1 j} Y_{i_1 k} Y_{i_2 j} Y_{i_2 k}\Big)\\
        =&
        \myE\sum_{j<k<i}Y_{ij}^2 Y_{ik}^2
        +2\myE\sum_{j<k<i_2<i_1}Y_{i_1 j} Y_{i_1 k} Y_{i_2 j} Y_{i_2 k}.\\
    \end{aligned}
    \end{equation*}
    By (ii) of Lemma~\ref{lemma:zhong1}, we have
    \begin{equation*}
    \begin{aligned}
        \myE\sum_{j<k<i}a_{ij}^2 a_{ik}^2 \leq
        \frac{n(n-1)(n-2)}{6}
        \myE a_{12}^4\leq c^4\frac{2(3+\Delta)^2(n-2)\mytr^2(\Lambda_2^2)}{3n(n-1)\mytr^2(\Sigma^2)}=o(1).
    \end{aligned}
    \end{equation*}
    By direct calculation, the second term equals
    \begin{equation*}
    \begin{aligned}
        &\myE\sum_{j<k<i_2<i_1}a_{i_1 j} a_{i_1 k} a_{i_2 j} a_{i_2 k}
        =\frac{c^4(n-2)(n-3) \mytr(\Lambda_2^4)}{6n(n-1)\mytr^2(\Sigma^2)}
        \leq
        \frac{c^4\lambda_{r+1}^2(\Sigma)}{6\mytr(\Sigma^2)}
        =o(1).
    \end{aligned}
    \end{equation*}
    This proves~\eqref{lemma2R1}. 

It follows from (ii) of Lemma~\ref{lemma:zhong1} that
$$
\myE\sum_{j<i} a_{ij}^4=\frac{n(n-1)}{2} \myE a_{12}^4
\leq c^4\frac{2(3+\Delta)^2\mytr^2(\Lambda_2^2)}{n(n-1)\mytr^2(\Sigma^2)}=o(1).
$$
Hence~\eqref{lemma2R2} holds.
And~\eqref{lemma2R3} follows similarly.

Now we prove~\eqref{lemma2R4}.
Note that
$$
\begin{aligned}
    &\sum_{j=1}^n \big(\sum_{i:i>j} a_{ij}b_i\big)^2
=
\sum_{j=1}^n \big(\sum_{i:i>j} a_{ij}^2 b_i^2+ 2 \sum_{i,k: i>k>j} a_{i j}a_{k j} b_{i}b_{k}\big)
\\
    =&
    \sum_{j<i} a_{ij}^2 b_i^2+ 2\sum_{j<k<i}  a_{i j}a_{k j} b_{i}b_{k}.
\\
\end{aligned}
$$
We have
...
\begin{aligned}
    &\myE a_{21}^2 b_2^2
    =
    \myE \myE [a_{21}^2 b_2^2|Z_2]
    =c^2 \big(\frac{n^2(n-1)}{2}\mytr(\Sigma^2)\big)^{-1} \myE [Z_2^T Q_2 \Lambda_2^2 Q_2^T Z_2 Z_2^T Q_1 t t^T Q_1^T Z_2]\\
    \leq&
    c^2 \big(\frac{n^2(n-1)}{2}\mytr(\Sigma^2)\big)^{-1} 
    \big(\myE (Z_2^T Q_2 \Lambda_2^2 Q_2^T Z_2)^2\big)^{1/2} \big(\myE(Z_2^T Q_1 t t^T Q_1^T Z_2)^2\big)^{1/2}\\
    \leq&
    (3+\Delta) c^2  \|t\|^2 \frac{2}{n^2(n-1)} 
    \frac{\mytr (\Lambda_2^2)}{\mytr(\Sigma^2)}.
\end{aligned}
$$
On the other hand, $
    \myE [a_{31} a_{21} b_3 b_2]
    =
    \myE \big[a_{21}b_2\myE [a_{31}  b_3 |Z_1,Z_2]\big]=0$.
    Thus,

$$
\begin{aligned}
    \myE\Big(\sum_{j<i} a_{ij}^2 b_i^2+ 2\sum_{j<k<i}  a_{i j}a_{k j} b_{i}b_{k}\Big)\leq
     \frac{(3+\Delta) c^2  \|t\|^2}{n} 
    \frac{\mytr (\Lambda_2^2)}{\mytr(\Sigma^2)}=o(1).
\end{aligned}
$$
Finally,~\eqref{lemma2R5} follows from
$$
        \myE\big(\sum_{i=1}^n b_i^4\big)=n \myE (b_1^4)
=\frac{1}{n} \myE (Z_1^T Q_1 t t^T Q_1^T Z_1)^2
\leq \frac{(3+\Delta)\|t\|^4}{n}=o(1).
$$



    First we deal with~\eqref{lemma2Rn}. For the first term of ~\eqref{lemma2Rn}, we have
$$
    \myE \big(\sum_{j<i} Y_{ij}^2\big)= \frac{\mytr(\Lambda_2^2)}{\mytr(\Sigma^2)}=(1-\sum_{i=1}^r \kappa_i^2)+o(1).
$$
    We need to show $\myVar\big(\sum_{j<i} Y_{ij}^2)=o(1)$. 
...
\begin{proof}[\textbf{Proof of Theorem~\ref{shaziCLT}}]
