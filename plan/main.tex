...
\sum_{j<i} Z_i^T \Gamma^T \Gamma Z_j
    ...
    \frac{2T(X_1,\ldots,X_n)-n(n-1)\|\mu\|^2}{n\sqrt{\mytr(\Sigma^2)}}
    =\frac{
2\sum_{j<i} Z_i^T \Gamma^T \Gamma Z_j
}{n\sqrt{\mytr(\Sigma^2)}}
    +
...
\begin{proposition}\label{prop:spiked2}
    ...
    we have
    $$
    \frac{\sum_{j<i} Z_i^T \Gamma^T \Gamma Z_j }{n\sqrt{\mytr(\Sigma^2)}}
    \xrightarrow{w}\frac{1}{2}\sum_{i=1}^r \kappa_i (\xi_i^2-1)+\kappa^* \xi_{r+1},
    $$
    where $\xi_1,\ldots \xi_{r+1}$ are iid standard normal random variables and $\kappa^*=\sqrt{1-\sum_{i=1}^r \kappa_i^2}$.
\end{proposition}
...
for some $C>0$,
     ...
     $$
     \begin{aligned}
         &\myVar\Big(\frac{\sum_{j=1}^{n} (n-j) Z_{j}^T Q_2 \Lambda_2^2 Q_2^T  Z_{j}}{\frac{n(n-1)}{2}\mytr \Lambda_2^2}\Big)\leq
         C\frac{2(2n-1)}{3(n-1)n}
         \frac{\mytr(\Lambda_2^4)}{\mytr^2(\Lambda_2^2)}
         \leq
     ...
     \begin{aligned}
\sum_{j=1}^{n} (n-j) Z_{j}^T Q_2 \Lambda_2^2 Q_2^T  Z_{j}=
         \frac{n(n-1)}{2}\mytr (\Lambda_2^2) (1+o_P(1)).
     \end{aligned}
     $$
...
\myE \big(Z_1^T Q_2 \Lambda_2^2 Q_2^T Z_2 
    ...
    \myE\Big(\frac{
    \sum_{k<j} (n-j)Z_j^T Q_2 \Lambda_2^2 Q_2^T Z_k 
}{\frac{n(n-1)}{2}\mytr (\Lambda_2^2)}\Big)^2=O\Big(\frac{\mytr(\Lambda_2^4)}{\mytr^2(\Lambda_2^2)}\Big)\to 0.
     $$
     Thus
     $$
         \sum_{i=2}^n \Big(\big(\sum_{j=1}^{i-1} Z_{j}\big)^T Q_2 \Lambda_2^2 Q_2^T \sum_{j=1}^{i-1} Z_{j}\Big)
     ...
     \begin{aligned}
         ...
         8c^2  
         \big(\frac{n(n-1)}{2} \mytr(\Lambda_2^2)\big)^{-2}
    ...
    Let $Y_i=V_{\Gamma}^T Z_i$, $i=1,\ldots,n$.
    Then
$$
...
$$
 where $Y_{i,1}$, $Y_{i,2}$ are the first $r$ and last $p-r$ elements of $Y_i$ respectively
...
For two matrices, $B_1$ and $B_2$, of the same dimension, let $B_1\circ B_2$ denote the Hadamard product of $B_1$ and $B_2$.
\begin{lemma}[\citet{songxi2010}, Proposition A.1]\label{lemma:zhong1}
    Under the conditions~\eqref{chenC1} and~\eqref{chenC2}, we have
    \begin{enumerate}[(i)]
        \item
            ...
            \end{equation}
        \item let $A=\Gamma^T \Gamma$. Then
            \begin{equation}\label{zhong2}
            \myE (Z_1^T A Z_2)^4= 3\mytr^2 (\Sigma^2)
            +6\mytr (\Sigma^4)+ 6\Delta \mytr(A^2 \circ A^2)+\Delta^2 \sum_{i,k=1}^m (A_{ik})^4;
            \end{equation}
        \item for any $m\times m$ positive definite matrix $B$, there exists a constant $C$ such that
            \begin{equation}\label{zhong2}
            \myE \big(Z_1^T B Z_1 -\mytr (B)\big)^4\leq C\mytr^2 (B^2);
            \end{equation}
        \item let $A_{ik}$ be the $(i,k)$th entry of $A=\Gamma^T\Gamma$. Then $\sum_{i,k} (A_{ik})^4\leq \mytr(\Sigma^4)$.
    \end{enumerate}
\end{lemma}
